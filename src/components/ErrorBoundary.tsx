    ...
    if (this.state.error) {
      ...
      return (
        <div className="rounded-xl border border-red-900/50 bg-red-950/20 px-6 py-5 text-center">
          ...
          <div className="mt-1 text-sm text-red-400/70">{this.state.error.message}</div>
          <button
            onClick={this.reset}
            className="mt-3 rounded-md border border-red-700/60 bg-red-900/30 px-3 py-1 text-sm font-medium text-red-200 hover:bg-red-900/50 transition-colors"
          >
            Try again
          </button>
        </div>

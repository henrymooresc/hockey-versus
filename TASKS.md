...
- [x] **Weight penalties by severity in the rivalry score.** `versus_stats` now stores penalty *minutes* rather than a count, and `CATEGORY_WEIGHTS.penaltyMinutes` is 2 per minute. A 2-minute minor still contributes 4, exactly as the old per-penalty weight did, so ordinary pairs are unchanged. A 5-minute fight now contributes 10 and a 10-minute misconduct 20. The skater prior was re-derived to 5.59.
    - **The effect on rankings is very small, and that is a data limit rather than a bug.** A penalty only counts for a pair when one player committed it and the *other drew it*. Only 6,995 of 910,828 pairs (0.77%) have any penalty minutes between them, and the maximum across 10 seasons is 20. On the all-time skater board only 7 of the top 200 pairs have any penalty minutes at all, the most being 4.
    - The heavily penalised pairs are real but do not rank. Lee and Tkachuk have 20 penalty minutes over 20 shared games — the change gains them about 1.2 weighted volume per game, on a board where rank 200 sits near 12.
- [ ] **Broaden what counts as a shared penalty, if you want fighting to move the rankings.** The current rule needs an exact committed-by and drawn-by match. A wider rule — both players on the ice for a penalty involving either of them — would capture far more, and pairs that fight would start to surface. This changes what the score means, so it is a product decision, not a tuning one.
